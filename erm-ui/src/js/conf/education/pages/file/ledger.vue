      ...
      <Card :padding="10">
        <div class="form-con">
          ...
          <Row :gutter="10">
            ...
            <Col span="6">
              <Select v-model="type" @change="getList(true)">
                <Option :value="1" :selected="true">县内</Option>
                <Option :value="2">县外</Option>
              </Select>
            </Col>
          </Row>
        </div>
        <div class="form-con" v-if="type===1">
          <Row :gutter="10">
        ...
        <div class="form-con" style="padding-bottom: 20px;border-bottom: 1px dashed #d5d5d5;margin-bottom: 10px;">
          <Row :gutter="10">
            <Col span="2">&nbsp;</Col>
            <Col span="6">
              <Button type="primary" @click="getList(true)">查 询</Button>
              <Button type="primary" @click="templateOut()">下载模版</Button>
            </Col>
          </Row>
        </div>
        <div class="do-some clearfix" v-if="listData.length">
          <div class="f-fl">
            ...
            <Button type="primary" v-if="type===2" @click="excelFile = true">导入数据</Button>
        ...
        <Modal v-model="excelFile" title="导入数据" :ok-text="'确定导入'" @on-ok="importData" @on-cancel="excelFile = false">
          <div class="data-in">
            <h3>请按以下步骤进行数据导入</h3>

            <p>已填好数据，导入数据 <input type="file" id="excel"
                                 accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                                 name="excel"></p>
          </div>
        </Modal>
  ...
  export default {
    ...
    methods: {
      ...
      getSchoolList() {
        ...
        io.get(api.school.listByFundedId, {fundedId: projectId == xueqian ? yiwu : projectId}, (res) => {
          let data = res.data
      ...
      importData() {
        ...
        // this.$Spin.show()
        io.post(api.ledger.import_outside, formData, (res) => {
          this.$Spin.hide()
          let blob = res
      ...
      templateOut() {
        window.location.href = domain.host + '/static/ledger.xls'
      ...
      dataOut() {
        ...
        if (this.type === 1) {
          ...
          searchData = _.merge({}, this.page, {
            fundId: this.formList.fund[1],
            schoolId: this.formList.school,
          })
        } else {
          path = api.ledger.export_outside;

        }

        console.log(path)

        io.get(path, searchData, (res) => {
          let blob = res
          let reader = new FileReader()
          reader.readAsDataURL(blob)
          reader.onload = (e) => {
