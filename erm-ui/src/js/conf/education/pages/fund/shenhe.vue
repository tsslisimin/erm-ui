      ...
      <Card :padding="10">
        ...
        <div class="do-some clearfix">
          <div class="f-fl">
            ...
            <Button type="primary" @click="rejectMore()">批量退回</Button>
          </div>
        </div>
        ...
        <v-table v-if="listData.length" :nowrap="true" :items="listData" :header="headData">
          ...
          <template slot="d-header" slot-scope="props">
            <th :class="{left:props.idx == 0,center:props.idx > 0}"><p :sort-key="props.data.colCode"><span>{{props.data.colName}}</span><span
              class="sort"><i class="iconfont up">&#xe6f4;</i><i class="iconfont down">&#xe608;</i></span></p></th>
          </template>
          ...
          <template slot="t-body" slot-scope="props">
            <tr>
              <td style="width: 30px;" class="check-box"><input type="checkbox" :data-id="props.data.stu_id"
                                                                :data-fundid="props.data.funded_id"
                                                                style="margin-top: 5px;"></td>
              <td :class="{left:$index == 0,center:$index > 0}" v-for="(el, $index) in headData" :key="$index">
                {{props.data[el.colCode]}}
              </td>
              <td class="center">
                <Button type="primary" @click="verifyAgree(props.data.stu_id)">同意</Button>&nbsp;&nbsp;&nbsp;<Button
                type="error" @click="verifyReject(props.data.stu_id)">退回
              </Button>
              </td>
            </tr>
          </template>
        ...
        <div class="edit">
          <Modal v-model="rejectModal" title="退回" :ok-text="'退回'" @on-ok="rejectInfo" @on-cancel="rejectModal = false">
            <div class="edit-con">
