    ...
    <div class="con">
      <Card :padding="10">
          ...
          <Row :gutter="10">
            ...
            <Col span="6">
              <Select v-model="formList.subtype" placeholder="请选择">
                <Option value="112">学前教育资助</Option>
        ...
        <div class="do-some clearfix">
          <div class="f-fl">
            <Button type="primary" @click="modalOut = true">导出数据</Button> <Button type="primary" @click="modalFile = true">信息对比</Button>
            <Modal v-model="modalOut" title="导出数据" :ok-text="'确定'" @on-ok="dataOut" @on-cancel="modalOut = false">
              <div class="data-out">
                <h3>请选择需要导出的信息</h3>
                <div class="bd">
                  <CheckboxGroup v-model="outData">
                    <Checkbox v-for="(item, $index) in outList" :key="$index" :label="item.key" :disabled="item.check"><span>{{item.value}}</span></Checkbox>
                  </CheckboxGroup>
                </div>
              </div>
            </Modal>
        ...
        <v-table v-if="listData.length" :nowrap="true" :items="listData" :header="headData">
          ...
          <template slot="t-body" slot-scope="props">
            <tr>
              <td :class="{left:$index == 0,center:$index > 0}" v-for="(el, $index) in headData" :key="$index"><span v-if="el.colCode != 'name'">{{props.data[el.colCode]}}</span><router-link v-if="el.colCode == 'name'" :to="{ name: 'detail', params: { stuId: props.data.stu_id, fundId: props.data.funded_id } }">{{props.data[el.colCode]}}</router-link></td>
            </tr>
          </template>
        </v-table>
