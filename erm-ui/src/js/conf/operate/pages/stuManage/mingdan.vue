<template>
  <div class="wrapper">
    <Location>学生管理 > 学生信息录入</Location>
    <div class="con">
      <Card :padding="10">
        <Form ref="formStu" :model="formStu" :rules="ruleStu" :label-width="0"
          ...
          <FormItem style="margin-bottom: 20px;">
            <Row :gutter="10">
              ...
              <Col span="3">
                <FormItem>
                  <Input v-model="formStu.school" placeholder="请输入"></Input>
                </FormItem>
              </Col>
              ...
              <Col span="5">
                <FormItem>
                  <Select v-model="formStu.accountType" placeholder="请选择">
                    <Option v-for="(item, $index) in formData.accountType" :value="item.dictCode" :key="$index">
                      {{item.dictName}}
                    </Option>
                  </Select>
                </FormItem>
              ...
              <Col span="5">
                <FormItem>
                  <Select v-model="formStu.nature" filterable placeholder="请选择">
                    <Option v-for="(item, $index) in formData.nature" :value="item.dictCode" :key="$index">
                      {{item.dictName}}
                    </Option>
                  </Select>
                </FormItem>
              ...
              <Col span="5">
                <FormItem>
                  <Select v-model="formStu.politicalStatus" placeholder="请选择">
                    <Option v-for="(item, $index) in formData.politicalStatus" :value="item.dictCode" :key="$index">
                      {{item.dictName}}
                    </Option>
                  </Select>
                </FormItem>
              ...
              <Col span="5">
                <FormItem>
                  <Select v-model="formStu.healthStatus" placeholder="请选择">
                    <Option v-for="(item, $index) in formData.healthStatus" :value="item.dictCode" :key="$index">
                      {{item.dictName}}
                    </Option>
                  </Select>
                </FormItem>
              ...
              <Col span="5">
                <FormItem>
                  <Select v-model="formStu.isMove" placeholder="请选择">
                    <Option v-for="(item, $index) in formData.isTrue" :value="item.dictCode" :key="$index">
                      {{item.dictName}}
                    </Option>
                  </Select>
                </FormItem>
              ...
              <Col span="5">
                <FormItem>
                  <Select v-model="formStu.addressType" placeholder="请选择">
                    <Option v-for="(item, $index) in formData.addressType" :value="item.dictCode" :key="$index">
                      {{item.dictName}}
                    </Option>
                  </Select>
                </FormItem>
          ...
          <FormItem style="margin-top: 30px;">
            <Row :gutter="10">
              <Col span="24" style="text-align: center;">
                <Button type="primary" @click="submit('formStu')">提交信息</Button>&nbsp;&nbsp;&nbsp;<Button type="ghost"
        ...
        <div class="do-some clearfix">
          <div class="f-fl">
            <Modal v-model="modalFile" title="导入数据" :ok-text="'导入数据'" @on-ok="dataIn" @on-cancel="modalFile = false">
              <div class="data-in">
                <h3>批量导入学生照片</h3>
                <p>选择已打包好照片并上传 <input type="file" id="filemore" style="line-height: 20px;" accept="*" name="file"
                                      @change="getFileMore()"></p>
                <p style="color: #f68f70;border-bottom: 1px dashed #d5d5d5;padding-bottom: 10px;margin-bottom: 10px;">
                  注：批量上传的照片需要打包上传，照片命名格式为“身份证号”</p>
                <h3>请按以下步骤进行数据导入</h3>
  ...
  export default {
    ...
    data() {
      return {
        ...
        lodgings: [
          {code: '否', des: '否'}, {code: '是', des: '是'}
        ],
        ruleStu: {
          name: [{required: true, message: '请输入姓名', trigger: 'blur'}],
          idcard: [
            {required: true, message: '请输入身份证号', trigger: 'blur'},
            {type: 'string', message: '身份证号长度不能小于18位', trigger: 'blur'}
          ],
          telphone: [
          ...
          isPoor: [{required: false, message: '贫困类型 ', trigger: 'change'}],
        },
        formData: {
          schSystem: [],
          stuType: [],
    ...
    methods: {
      ...
      submit(name) {
        this.$refs[name].validate(valid => {
          if (valid) {
            let date = this.formStu.startYear ? new Date(this.formStu.startYear) : ''
            io.post(api.student.saveStudent, {
              ...
              school: this.formStu.school
            }, (res) => {
              this.$Message.success('学生信息录入成功！')
            })
          } else {
      ...
      // 批量上传图片
      getFileMore() {
        let file = document.getElementById('filemore')

        this.fileDataMore = new FormData()
        this.fileDataMore.append('file', file.files[0])

        this.$Spin.show()
        io.post(api.student.importPic, this.fileDataMore, (res) => {
          this.$Spin.hide()
          this.$Message.info('上传照片成功' + res.data.resultData.successNum + '张，失败' + res.data.resultData.failNum + '张')
        }, (e) => {
          this.$Spin.hide()
          this.$Message.error('批量照片上传出错！')
        }, (e) => {
          this.$Spin.hide()
          this.$Message.error('批量照片上传出错！')
        })
      ...
      downFile() {
        ...
        window.location.href = domain.host + '/static/studentImportModel.xls'
        // io.get(domain.host + '/static/studentImportModel.xls', (res) => {
        //   let blob = res
        //   let reader = new FileReander()
        //   reader.readAsDataURL(blob)
        //   reader.onload = (e) => {
        //     let a = document.createElement('a')
        //     a.download = 'studentImportModel.xls'
        ...
        // $(`<form action="http://erm.comezx.com/static/studentImportModel.xls" method="get"></form>`).appendTo('body').submit().remove()
      },
      // 上传文件
      getFiles() {
      ...
      dataIn() {
        ...
        this.$Spin.show()
        io.post(api.student.import, this.formDatas, (res) => {
          this.$Spin.hide()
          ...
          if (res.data.resultData.failList && res.data.resultData.failList.length) {
            ...
            this.$Message.info({
              content: msg + '  ' + res.data.resultData.failMsg,
              duration: 10
            })
          }
        }, (e) => {
