            ...
            <Row>
              ...
              <Col span="12" v-for="(item, $index) in formDatas" :key="$index">
                <FormItem :label-width="140" :label="item.ermFieldEntity.fieldName" style="margin-bottom: 20px;">
                  <Input v-if="item.ermFieldEntity.type == 1" v-model="item.ermFieldEntity.value" placeholder="请输入"
                         style="width: 300px;"></Input>
                  <Input v-if="item.ermFieldEntity.type == 2" v-model="item.ermFieldEntity.value" placeholder="请输入"
                         style="width: 300px;"></Input>
                  <Select v-if="item.ermFieldEntity.type == 3" filterable v-model="item.ermFieldEntity.value"
                          placeholder="请选择" style="width: 300px;">
                    <Option v-for="el in item.ermFieldEntity.fieldVal" :key="el.id" :value="el.valKey">{{el.valVal}}
                    </Option>
                  </Select>
                  <Select v-if="item.ermFieldEntity.type == 4" multiple filterable v-model="item.ermFieldEntity.value"
                          placeholder="请选择" style="width: 300px;">
                    <Option v-for="el in item.ermFieldEntity.fieldVal" :key="el.id" :value="el.valKey">{{el.valVal}}
                    </Option>
                  </Select>
    ...
    methods: {
      ...
      getForm(id) {
        io.get(api.school.querySchField, (res) => {
          ...
          this.formDatas = data
          this.formUrl = baseUrl + '/family/index.html?schId=' + this.formDatas[0]['schId']
        })
      },
      // input 工具方法
      generatorInput(obj) {
